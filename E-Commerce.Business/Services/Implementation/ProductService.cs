...
namespace E_Commerce.Business.Services.Implementation
{
    public class ProductService : IProductService
    {
        ...
        public async Task<ProductFilterResultViewModel> GetFilteredProductsAsync(ProductFilterViewModel filter)
        {
            // Get all products with includes
            var productsQuery = _unitOfWork.Products.GetAllQueryable("Category,Reviews");
            ...
            {
                productsQuery = productsQuery.Where(p => p.Name.Contains(filter.Search) || (p.Description != null && p.Description.Contains(filter.Search)));
            }
            ...
            var inStockCount = productsQuery.Count(p => p.StockCount > 0);
            var outOfStockCount = totalProducts - inStockCount;
            ...
            // Apply pagination
            int pageSize = Numbers.DefaultPageSize - 7;
            var products = productsQuery.Skip((filter.Page - 1) * pageSize).Take(pageSize).ToList();
            ...
            var paginatedProducts = new PaginatedList<ProductViewModel>(productsVM, totalProducts, filter.Page, pageSize);

            return new ProductFilterResultViewModel
            {
